...
def get_name():
    con = create_connection()
    cur = con.cursor()
    query = "select value from memory where name = 'assistent_name'"
    cur.execute(query)
...
def update_name(new_name):
    con = create_connection()
    cur = con.cursor()
    query = "update memory  set value ='" + new_name + "' where name = 'assistent_name'"
    cur.execute(query)
    con.commit()
...
def turn_on_speech():
    if check_internet_connection():
        ...
        return "Hey please turn on the internet first."
